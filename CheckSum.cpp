...
int main() {
  ...
  const int quickmath = N/16;
  TStopwatch timerSIMD;
    // TODO
  for( int ii = 0; ii < NIter; ii++ ){
    fvec fSolutionOfTemplateSum = Sum <fvec>(reinterpret_cast<fvec*>(str), quickmath);
    unsigned char *pChar = reinterpret_cast<unsigned char*>(&fSolutionOfTemplateSum);
    for( int j = 0; j < 16; j++ ){
      sumV ^= pChar[j];
    }
  }
  ...
  const int intmath = N/4;
  TStopwatch timerINT;
    // TODO
  for( int ii = 0; ii < NIter; ii++ ){
    int fSolutionOfTemplateSum = Sum <int>(reinterpret_cast<int*>(str), intmath);
    unsigned char *pChar = reinterpret_cast<unsigned char*>(&fSolutionOfTemplateSum);
    for( int j = 0; j < 4; j++ ){
      sumI ^= pChar[j];
    }
