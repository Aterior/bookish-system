...
int main() {
  ...
  for(int io=0; io<NIterOut; io++)
  {
	...
	tmpA[j] = dataSIMD1.data[float_v::Size*i+j].a;
	tmpB[j] = dataSIMD1.data[float_v::Size*i+j].b;
	tmpC[j] = dataSIMD1.data[float_v::Size*i+j].c;
      }

        float_v tmpdet = tmpB*tmpB - 4*tmpA*tmpC;
        float_v tmpX   = (-tmpB+sqrt(tmpdet))/(2*tmpA);
      
      for(int k = 0; k < float_v::Size; k++){
	dataSIMD1.data[float_v::Size*i+k].x = tmpX[k];
      }
    }
  }
  timerSIMD1.Stop();
  ...
  for(int io=0; io<NIterOut; io++)
    ...
    {
      // TODO
      float_v &tmpA2 = reinterpret_cast<float_v&>(dataSIMD2.a[i]);
      float_v &tmpB2 = reinterpret_cast<float_v&>(dataSIMD2.b[i]);
      float_v &tmpC2 = reinterpret_cast<float_v&>(dataSIMD2.c[i]);
      float_v &tmpX2 = reinterpret_cast<float_v&>(dataSIMD2.x[i]);

      const float_v tmpdet2 = tmpB2*tmpB2 - 4*tmpA2*tmpC2;
      tmpX2 = (-tmpB2+sqrt(tmpdet2))/(2*tmpA2); 

     // for(int k = 0; k < float_v::Size; k++){
       // float &tmpA2 = reinterpret_cast<float&>(dataSIMD2.a[k]);
       // float &tmpB2 = reinterpret_cast<float&>(dataSIMD2.b[k]);
       // float &tmpC2 = reinterpret_cast<float&>(dataSIMD2.c[k]);
        //tmpX2[k] = (-tmpB2+sqrt(tmpdet2))/(2*tmpA2);
	//float tmpX2 = (-tmpB2+sqrt(tmpdet2))/(2*tmpA2);

	//dataSIMD2.x = reinterpret_cast<float*>(&tmpX2);
      //}
      //dataSIMD2.x = reinterpret_cast<float_v>(tmpX2);
    }
  timerSIMD2.Stop();
  ...
  for(int io=0; io<NIterOut; io++)
    ...
    {
      // TODO
      float_v &tmpA3 = reinterpret_cast<float_v&>(dataSIMD3.data[i].a[0]);
      float_v &tmpB3 = reinterpret_cast<float_v&>(dataSIMD3.data[i].b[0]);
      float_v &tmpC3 = reinterpret_cast<float_v&>(dataSIMD3.data[i].c[0]);
      float_v &tmpX3 = reinterpret_cast<float_v&>(dataSIMD3.data[i].x[0]);
      const float_v tmpdet3 = tmpB3*tmpB3 - 4*tmpA3*tmpC3;
      tmpX3 = (-tmpB3+sqrt(tmpdet3))/(2*tmpA3); 
    }
  timerSIMD3.Stop();
